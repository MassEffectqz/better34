  ...
  try {
    ...
    return raw
      .map(e => typeof e === 'string'
        ...
        : { q: String(e.q || ''), count: Math.max(1, e.count | 0), pin: !!e.pin })
      .filter(x => x.q);
  } catch { return []; }
}
...
function addHistory(q) {
  if (!q) return;
  const h = getHistory();
  const ex = h.find(x => x.q === q);
  if (ex) ex.count++;
  else h.unshift({ q, count: 1, pin: false });
  saveHistory(h.slice(0, 30));
}
function removeHistory(q) { saveHistory(getHistory().filter(x => x.q !== q)); }
function togglePinHistory(q) {
  const h = getHistory();
  const e = h.find(x => x.q === q);
  if (!e) return false;
  e.pin = !e.pin;
  saveHistory(h);
  return e.pin;
}
// Очистка оставляет закреплённые запросы.
function clearHistory() { saveHistory(getHistory().filter(x => x.pin)); }

// esc используется и в текстовых, и в атрибутных контекстах (data-query=...):
// браузер экранирует &, <, >, но НЕ кавычки — добавляем их сами.
function esc(s) {
  const d = document.createElement('div');
  d.textContent = s;
  return d.innerHTML.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}
function go(promise) { if (promise && promise.catch) promise.catch(err => console.error(err)); }
